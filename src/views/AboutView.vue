<template>
	<main class="my-8">
		<h1 class="max-w-screen-xl mx-auto bg-gradient-to-r from-green-600 to-blue-400 p-4 md:text-3xl text-white text-center">
			IRON HACK FINAL PROJECT
		</h1>
		<section class="flex my-2 text-xl">
			<div class="mx-auto w-full">
				<div class="max-w-screen-xl mx-auto my-4 flex-col bg-gradient-to-l from-sky-500 to-gray-700 text-center p-2 text-white">
					<h1 class="md:text-3xl mb-2">
						Joan Vila Valls
					</h1>
					<a class="text-yellow-300 hover:text-yellow-400 hover:underline"
						href="https://www.linkedin.com/in/joanvv/">LinkedIn Profile link</a>
				</div>
				<div class="flex justify-center my-6 bg-white max-w-screen-sm mx-auto">
					<a class="font-bold mx-8 hover:underline text-xl md:text-2xl hover:text-gray-500"
						href="https://todo-app-by-jvv.netlify.app/">
						Live website
					</a>
					<a class="font-bold mx-8 hover:underline hover:text-gray-500 text-xl md:text-2xl"
						href="https://github.com/joanojan/TO-DO_app">
						GitHub repo
					</a>
				</div>
				<section class="flex-col my-4">
					<h2 class="max-w-screen-sm mx-auto bg-white text-center font-bold text-2xl">Single Page Application</h2>
					<div class="flex justify-center my-4">
						<div class="flex-col">
							<table class="table-auto">
								<h3 class="my-4 font-bold text-2xl text-center bg-yellow-500">Frontend stack:</h3>
								<tbody class="bg-gray-700 opacity-75 font-bold text-xl text-white">
									<tr>
										<td class="px-4 py-2">Framework:</td>
										<td class="px-4 py-2">
											<a class="hover:underline" href="https://vuejs.org/">Vue3</a>
										</td>
									</tr>
									<tr>
										<td class="px-4 py-2">Build tool:</td>

										<td class="px-4 py-2">
											<a class="hover:underline" href="https://vitejs.dev/">Vite</a>
										</td>
									</tr>
									<tr>
										<td class="px-4 py-2">Routing:</td>

										<td class="px-4 py-2">
											<a class="hover:underline" href="https://router.vuejs.org/">Vue Router</a>
										</td>
									</tr>
									<tr>
										<td class="px-4 py-2">State management:</td>
										<td class="px-4 py-2">
											<a class="hover:underline" href="https://pinia.vuejs.org/">Pinia</a>
										</td>
									</tr>
									<tr>
										<td class="px-4 py-2">CSS framework:</td>
										<td class="px-4 py-2">
											<a class="hover:underline" href="https://tailwindcss.com/">Tailwind</a>
										</td>
									</tr>
								</tbody>
							</table>
						</div>
					</div>
				</section>
			</div>
		</section>
	</main>
</template>